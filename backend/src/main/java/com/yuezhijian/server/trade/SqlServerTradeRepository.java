...
public class SqlServerTradeRepository implements TradeRepository {
    ...
    public Optional<BillDetail> findById(long id) {
        ...
        return bill == null ? Optional.empty()
                : Optional.of(new BillDetail(
                        bill, mapper.findLines(id), mapper.findPayments(id), mapper.findHistory(id)));
    }
    ...
    public BillDetail addLine(AddBillLineCommand command) {
        ...
        BigDecimal total = current.bill().receivableAmount().add(command.line().amount());
        ...
        insertLine(
                command.billId(), current.lines().size() + 1, current.bill().storeId(), command.line());
