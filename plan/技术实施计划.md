...
| 计划状态 | 已确定芋道完整单体后端 + Vue3 管理端为重构底座 |
...
> 2026-07-30 架构校正：新的运行底座已经确定为 `ruoyi-vue-pro master-jdk17@ec3f7cb` + `yudao-ui-admin-vue3 master@9445977`。本文中仍出现的自研 `/api/v1`、Spring Session 和旧目录细节属于 `main@6cae2c8` 设计资产，用于业务迁移对照；与当前工程冲突时，以 `docs/芋道底座重构迁移方案.md` 和 `ITER-53` 为准。
...
| 后端框架 | 芋道 `ruoyi-vue-pro` 单体版 | Spring Boot 3.5.15，`master-jdk17` | 直接复用 system、infra 以及按需启用的 member、bpm、pay、report、ERP/WMS 模块 |
| 安全与会话 | 芋道 Spring Security Token + Redis | 跟随芋道 2026.06 | 复用访问/刷新 Token、强制下线、多终端认证、动态菜单和权限缓存 |
...
2. 芋道底座之后的新 SQL 统一放在 `backend/yudao-server/src/main/resources/db/migration/`，命名为 `VYYYYMMDDHHMM__description.sql`；main 的旧脚本已归档在 `ref/main-backend-snapshot/backend/src/main/resources/db/migration/`，不得直接重放。
...
7. 应用启动前执行 `flyway validate`，再执行 `migrate`；`baseline-on-migrate` 只服务于本地一次性芋道基线，生产环境使用独立配置禁用 `clean` 和自动基线，Migration失败时停止发布。
...
1. 管理端沿用芋道 `/admin-api`；App 端沿用 `/app-api`。main 的 `/api/v1` 路径只作为迁移映射来源，不继续扩展。
...
1. PC 管理端使用芋道 Spring Security 不透明 Token 与 Redis 会话，支持访问 Token、刷新 Token、强制下线和多终端管理；生产只允许 HTTPS，Token 不写日志。
...
├── .gitignore
├── Makefile
├── infra/compose.yaml              # SQL Server、Redis、MinIO
