...
> 重构说明：本文主体记录 `main@6cae2c8` 已实现和规划的业务契约，是迁移需求清单，不再代表当前运行路径。芋道管理端使用 `/admin-api`、Token 和原生统一响应；迁移每个接口时必须补充“旧接口 → 芋道模块新接口”映射，禁止继续扩展旧 Session/CSRF 协议。
...
- 门店数据范围：会员、预约、账单、储值/积分、次卡、会员代金券、整单冲销、提成、回访、服务反馈、归属调整及员工/工位/服务项目接口已由业务层校验列表条件、详情对象、写入目标及幂等返回；其他存量模块接入前不得仅依赖前端门店选择器作为安全边界。
...
| API-CAT-011 | `POST /services/import`、`POST /exports` | 当前门店UTF-8 CSV导入；`exportType=SERVICE_CATALOG`及关键词导出 | 导入返回202任务，最多5000行并生成逐行结果；导出需`catalog:service:export`；均已实现 | 优化系统管理-01 |
...
| API-CFG-006 | `GET /banners`、`POST /banners`（multipart） | 按展示位置、名称、状态查询；以JSON配置部分和JPG/PNG/WEBP文件新建，含标题、跳转类型/地址、排序和有效期 | 管理列表/完整记录；`system:banner:view/manage`，图片最大10 MiB且校验文件头；已实现 | 系统管理-32 |
| API-CFG-007 | `GET/POST /color-style-categories`、`GET/PUT /color-style-categories/{id}` | 编码/名称查询；父分类、分类编码、名称、排序、状态、version | 分层分类列表/并发安全更新；编码创建后不可修改，防止自身/后代循环；`system:color-style:view/manage`；已实现 | 系统管理-22 |
...
| API-CFG-010 | `GET /banners/active`、`GET /banners/active/{id}/image`、`GET /banners/{id}`、`GET /banners/{id}/image` | 工作台按位置读取当前启用且在有效期内的最小展示信息及私有图片；管理端读取详情和预览 | 活动图片不返回文件ID/操作人等管理字段；图片经权限校验、摘要校验和ETag响应，不暴露对象键；已实现 | 系统管理-32 |
| API-CFG-011 | `PUT /banners/{id}`、`PUT /banners/{id}/image`（multipart） | 修改位置、标题、跳转、排序、有效期、状态；使用version单独换图 | rowversion并发安全更新；站外只允许HTTPS，换图后旧文件软删除并写审计；已实现 | 系统管理-32 |
| API-CFG-012 | `PUT /color-style-categories/{id}/image`（multipart）、`GET /color-style-categories/{id}/image` | version、JPG/PNG/WEBP分类图片 | 分类图片独立换图和鉴权预览；最大10 MiB、文件头及摘要校验，不暴露对象键；已实现 | 系统管理-22 |
| API-CFG-013 | `GET/POST /color-styles`、`GET/PUT /color-styles/{id}` | 分类、色号/名称、状态、page/size查询；色号、名称、说明、排序、1~20个分类、状态、version | 最大100条分页、色号完整记录和多分类关系；编码创建后不可改、rowversion更新和审计；已实现 | 系统管理-22 |
| API-CFG-014 | `POST /color-styles/{id}/assets`（multipart）、`PUT /color-styles/{id}/assets/{assetId}`、`GET /color-styles/{id}/assets/{assetId}/content` | 图片、排序；素材状态和version | 每色号最多20张启用素材，私有鉴权预览、并发安全更新和审计；已实现 | 系统管理-22 |
...
首页图片展示位置固定为`PC_HOME/HOME_SERVICE_HOME`，跳转类型固定为`NONE/INTERNAL/EXTERNAL`。站内地址必须以单个`/`开头，站外地址必须是无用户凭据的完整HTTPS URL。配置停用或不在`validFrom~validTo`范围时，工作台接口和活动图片接口均不可读取；管理预览仍可查看停用配置。当前合同没有确认统一裁剪比例，因此只做文件类型、文件头和大小校验，不伪造裁剪验收。

线上试色结构以旧系统`ItemColorStyleGroups`、`ItemColorStyles`、`ItemColorStyleGroupAssigns`映射及管理/WAP页面为依据：分类可有父级和分类图片，一个色号可归属多个分类并挂多张展示素材。分类与色号编码创建后固定，启用色号只能选择启用分类；停用分类前必须先处理启用子分类。合同包含“导入”，但旧源码未发现导入入口或字段模板，甲方确认文件格式、图片打包方式及覆盖规则前不提供临时导入接口，也不将本项标记为全量完成。
